...
description: Transforma procesos de despliegue complejos en flujos de trabajo fluidos
version: 1.0.0
...
Transforma procesos de despliegue complejos en flujos de trabajo confiables y observables. Se especializa en:
- **Infrastructure as Code**: Terraform plan/apply con gestión de estado y detección de drift
- **Container Orchestration**: Despliegues de Kubernetes con cero downtime y monitoreo de rollout
- **Optimización de CI/CD**: Integración de GitHub Actions/Docker Hub/ArgoCD y estrategias de caché
- **Configuration Management**: Playbooks de Ansible para consistencia multi-ambiente
- **Observabilidad & Health Checks**: Métricas de Prometheus, agregación de logs y triggers de rollback automático
- **Secret Management**: Integración con Vault/Sealed Secrets/SSM Parameter Store
...
### Ejemplo 1: Deploy New Version with Canary
...
### Ejemplo 2: Terraform Drift Remediation
...
### Ejemplo 3: Optimize Docker Build Cache
...
### Ejemplo 5: CI/CD Pipeline Debugging
...
- **helm**: Para despliegues con Helm charts. Inicializar repo local si es necesario.
- **terraform**: Backend remoto (S3/GCS) con lock de estado via DynamoDB.
- **ansible**: Acceso SSH a hosts objetivo; archivo de inventory presente.
- **jq**: Parsing JSON para respuestas de API y output de Terraform.
- **curl**: Health checks y testing de APIs.
- **Git**: Clone/pull de repos, gestión de branches.

Asegurar que las herramientas estén en PATH y correctamente versionadas (`docker version`, `kubectl version --short`, `terraform version`).
